...
public class DaoMesaImpl implements DaoMesa {
	...
	public List<Mesa> consultar() {
		...
		sql.append("SELECT ").append("id, n_salon, n_mesa, estado").append(" FROM mesa");
		try (Connection c = con.getConexion();
			...
			System.out.println("Error al consultar en DaoMesaImpl: "+e.getMessage());
		}
		return lista;
	}

	@Override
	public Mesa agregar(Mesa objeto) {
		StringBuilder sql = new StringBuilder();
		sql.append("INSERT INTO mesa(").append("id,").append("n_salon,").append("n_mesa").append(") VALUES (null,?,?)");
	...
	public boolean editar(Mesa objeto) {
		StringBuilder sql = new StringBuilder();
		sql.append("UPDATE mesa SET ").append("n_salon = ?,").append("n_mesa = ?,").append("estado = ?")
				.append(" WHERE id = ?");
		try (Connection c = con.getConexion(); PreparedStatement ps = c.prepareStatement(sql.toString());) {
			ps.setInt(1, objeto.getN_salon());
			ps.setInt(2, objeto.getN_mesa());
	...
	public boolean eliminar(int codigo) {
		StringBuilder sql = new StringBuilder();
		sql.append("DELETE FROM mesa ").append("WHERE id = ?");
		try (Connection c = con.getConexion(); PreparedStatement ps = c.prepareStatement(sql.toString());) {
			ps.setInt(1, codigo);
			return (ps.executeUpdate() != 0);
		} catch (Exception e) {
	...
	public Mesa obtener(int codigo) {
		Mesa mesa = null;
		StringBuilder sql = new StringBuilder();
		sql.append("SELECT ").append("id,").append("n_salon,").append("n_mesa,").append("estado").append(" FROM mesa")
				.append(" WHERE id = ?");
		try (Connection c = con.getConexion(); PreparedStatement ps = c.prepareStatement(sql.toString());) {
			ps.setInt(1, codigo);
