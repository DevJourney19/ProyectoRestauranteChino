...
public class MesaMozoProceso extends HttpServlet {
	...
	protected void proccesRequest(HttpServletRequest request, HttpServletResponse response)
		...
		// Cremamos la clase cliente
		Cliente cliente = new Cliente();
		// Creamos el daoCliente
		DaoCliente daoCli = new DaoClienteImpl();

		Cliente agregando = daoCli.agregar(cliente);
		cliente.setId(agregando.getId());
		// Tengo que ver la manera de poder obtener el ide del pedido que ha sido creado

		// POSTERIORMENTE SE PODRÁ EDITAR ESE PEDIDO EN ESPECIFICO
		pe.setMesa(mesa); // Se le asigna la mesa al pedido
		pe.setCreated_at(new Date());
